...
import com.mem.model.MemberServiceHibernate;
import com.mem.model.MemberVO;

@MultipartConfig(fileSizeThreshold = 1024 * 1024, maxFileSize = 5 * 1024 * 1024, maxRequestSize = 5 * 5 * 1024 * 1024)
public class MemberServletHibernate extends HttpServlet {
...
/*************************** 1.接收請求參數 - 輸入格式的錯誤處理 **********************/
			...
			if (!errorMsgs.isEmpty()) {
				RequestDispatcher failureView = req.getRequestDispatcher("/mem_hibernate/select_page.jsp");
...
/*************************** 3.查詢完成,準備轉交(Send the Success view) *************/
			...
			String url = "/mem_hibernate/listOneMem.jsp";
...
/*************************** 3.查詢完成,準備轉交(Send the Success view) ************/
			req.setAttribute("memberVO", memberVO); // 資料庫取出的empVO物件,存入req
			String url = "/mem_hibernate/update_mem_input.jsp";
...
/*************************** 1.接收請求參數 - 輸入格式的錯誤處理 **********************/
			...
			if (!errorMsgs.isEmpty()) {
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/mem_hibernate/update_mem_input.jsp");
				failureView.forward(req, res);
				return; // 程式中斷
			}
...
/*************************** 2.開始修改資料 *****************************************/
			MemberServiceHibernate memSvc = new MemberServiceHibernate();
			if (memImg != null) {
				memberVO = memSvc.updateMember(memNo, memName, memSex, memPhone, memEmail, memUid, memBirthday,
						memAccount, memPassword, memStatus, memImg);
			} else {
				memberVO = memSvc.updateWithoutImg(memNo, memName, memSex, memPhone, memEmail, memUid, memBirthday,
						memAccount, memPassword, memStatus);
			}
...
/*********************** 1.接收請求參數 - 輸入格式的錯誤處理 *************************/
			...
			if (!errorMsgs.isEmpty()) {
				req.setAttribute("memberVO", memberVO); // 含有輸入格式錯誤的empVO物件,也存入req
				RequestDispatcher failureView = req.getRequestDispatcher("/mem_hibernate/addMem.jsp");
				failureView.forward(req, res);
				return;
			}
...
/*************************** 2.開始新增資料 ***************************************/
			...
			memberVO = memSvc.addMember(memName, memSex, memPhone, memEmail, memUid, memBirthday, memAccount,
					memPassword, memStatus, memImg);

/*************************** 3.新增完成,準備轉交(Send the Success view) ***********/
			String url = "/mem_hibernate/listAllMem.jsp";
...
/******************************姓名模糊搜尋**************************************/
		...
		if ("findBySearching".equals(action)) { 

			List<String> errorMsgs = new LinkedList<String>();
			req.setAttribute("errorMsgs", errorMsgs);

/*************************** 1.接收請求參數 - 輸入內容確認 **********************/
			String str = req.getParameter("input"); // 若未輸入，則導向listAllMem.jsp
			if (str == null || (str.trim()).length() == 0) {
				String url = "/mem_hibernate/listAllMem.jsp";
...
/*************************** 2.開始查詢資料 *****************************************/
				...
				MemberServiceHibernate memSvc = new MemberServiceHibernate();
				List<MemberVO> list = memSvc.findBySearching(str);

/*************************** 3.查詢完成,準備轉交(Send the Success view) *************/
				req.setAttribute("list", list);
				String url = "/mem_hibernate/listBySearching.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);
				successView.forward(req, res);
			}
		}
